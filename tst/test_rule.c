...
void world_expected_after_movement(struct world *initial_world,
  ...
  if (dx == 0 && dy == 0) {
    ...
    unsigned int idx_mov =
        modulo(i + dx, HEIGHT) * WIDTH + modulo(j + dy, WIDTH);
...
void create_rule_movements(struct rule *r) {
  ...
  r->next_state[0].next_color =
      GRASS; // Le changement de couleur fait passer une cellule de SAND à GRASS
  r->next_state[7].dx = 2;
  r->next_state[7].dy = 0;
  r->next_state[8].dx = 0;
  r->next_state[8].dy = -2;
...
void evolution_world(struct world *w, struct rule *r, unsigned int idx_change) {
  ...
  for (int i = 0; i < WIDTH * HEIGHT; i++) {
    ...
    if (rule_match(w, r, p.x, p.y)) {
      ...
      if (dx || dy) {
        queue_append(&q, p.x, p.y,
                     1); // la rule avec l'index 1 est celle qui remplace une
  ...
  while (queue_is_not_empty(&q)) {
    ...
    if (change_tmp->idx_rule ==
        1) // Cas particulier du deplacement avec passage de i,j en EMPTY
    {
