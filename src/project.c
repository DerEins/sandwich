...
int solve_conflict(struct conflict t_conflicts[], unsigned int i,
                   unsigned int j) {
  struct conflict c = t_conflicts[i * WIDTH + j];
  if (c.conflict_to_process == 1) // si il n'y a qu'un 'conflit' (c'est plus un
                                  // conflit du coup mais bon ...)
  {
    return 1;
  } else if ((rand() % c.nb_conflicts) ==
             0) // 1 chance sur le nb de conflit d'accepter ce conflit
  {
    return 1;
  } else {
    return 0;
...
void world_apply_rule(struct world *w, struct rule *r, int i, int j,
  ...
  if (solve_conflict(t_conflicts, modulo(i + dx, HEIGHT),
                     modulo(j + dy, WIDTH))) {
    if (dx || dy) {
      w->t[i * WIDTH + j] = EMPTY;
      w->t[modulo(i + dx, HEIGHT) * WIDTH + modulo(j + dy, WIDTH)] =
          rule_change_to(r, idx_change);
    } else {
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < nb_pictures; i++) {
      ...
      for (unsigned int l = 0; l < WIDTH; l++) {
        for (unsigned int j = 1; j < rules_count(); ++j) {
          ...
          if (rule_match(&w, r, k, l)) {
            ...
            t_conflicts[index_tmp].nb_conflicts =
                t_conflicts[index_tmp].nb_conflicts + 1;
            t_conflicts[index_tmp].conflict_to_process =
    ...
    while (queue_is_not_empty(&q)) {
      ...
      world_apply_rule(&w, rule_get(change_tmp->idx_rule), change_tmp->i,
                       change_tmp->j, change_tmp->idx_next_state, t_conflicts);
    }
    world_disp(&w);
  }
  return 0;
